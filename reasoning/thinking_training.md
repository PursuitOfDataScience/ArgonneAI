...
## 17. Deep eval of intermix `363908`, and the WEIGHT-SOUP that clears the bar (2026-07-03 — pipeline now wired into weekend.sh/night.sh)

§16 said "keep grinding to the ~4B-token knee." This section is the deep eval that
**overturns that** and gives the decisive move: **stop the 16B grind, harvest the
banked math via a training-free weight-soup, and run the real downstream test.**
(This section absorbs the former standalone `reasoning/eval_intermix_363908_findings.md`.)

> **⭐ One line:** a training-free average `0.35·seed + 0.65·intermix363908`
> (both θ=1e6, same lineage → a clean linear interpolation) scores **MATH 15/20
> AND GEN 13/15** — the **first from-scratch Argonne base to clear the both-axes
> bar** (§15: MATH ≥14, GEN ≥13), assembled from checkpoints we already have.
> The midtraining's math gains are "banked," the both-axes base is reachable NOW,
> and finishing the 16B grind is unnecessary. **The soup downstream pipeline is
> now the default of `weekend.sh` / `night.sh` (`MODE=soup`).**

**The checkpoint.** `models/midtrain/checkpoint_step_363908.pt` (~1.41 B intermix
tokens; 50:50 through step 345108, **60:40 (GENERAL_RATIO=1.5)** after; LR still
**3e-4**), pinned during eval; the intermix run kept training. Seed =
`pretrain/checkpoint_step_329148.pt` = `argonne-3.0-base`. Pruning is off, so
`363908` persists for re-souping.

### The numbers (`reasoning/eval_intermix_base.py`, 1×H100, θ=1e6)

| model | MATH std/ext (·/20) | GEN std/ext (·/15) | NLL FineWeb | NLL FineMath | long-ctx ppl @8k–13.5k |
|---|---|---|---|---|---|
| seed 329148 (= argonne-3.0-base) | 3 / 2 | **14 / 15** | 2.572 | 3.322 | ~803 (collapses past 1024) |
| finemath 864124 (pure math) | **18 / 17** | 6 / 9 | 4.215 | **1.408** | 71 |
| **INTERMIX 363908** | **14 / 11** | **11 / 10** | **2.984** | **1.589** | **15** (flat) |
| — intermix @345108 (§16, 644 M tok) | 12 / 11 | 11 / 14 | 3.006 | 1.601 | ~17 (flat) |

**The bar (§15/§16 stop rule): MATH ≥14/20 AND GENERAL ≥13/15 on the standard set.**
→ raw `363908`: MATH **14 ✓ (soft — ext only 11, multi-step still fails)** ·
GENERAL **11 ✗ (misses by 2, and *trending down*: fresh-item ext regressed 14→10
vs 345108; real seed-facts lost — largest planet→"Mars", 1st president→"Lincoln")**.
So the raw checkpoint is a **CASE-B near-miss**, driven entirely by general.

- **Eval trustworthy** (audited, tripwires passed): the SEED `.pt` scored
  bit-identical to the `argonne-3.0-base` HF dir, and finemath 864124 reproduced
  §16 exactly, so the intermix numbers aren't a silent-crippled-load artifact.
  θ=1e6 is correct (flat long-ctx curve confirms; 1e4 would corrupt — §11 trap).
- **Diagnosis of the general erosion:** at **LR 3e-4** (the from-scratch pretraining
  LR) the accumulating math tokens over-write the seed's healthy 14/15 general
  *faster than 60:40 replay protects it*. The §15 lesson — a general-healthy base
  wants a **gentle** touch (Qwen/Llama used 5e-5) — showing up as erosion at 3e-4.
- **Efficiency win still holds (§16):** ~90% of pure-FineMath's math-NLL gain at
  ~25% of its general-NLL cost, in 1.41B tokens. Intermix also *is* the context
  extension (flat ppl to 13.5k; longmino obsolete).

### Is it worth continuing? Three cheap tests said: stop the grind, soup instead

**1. Forecast to the 16B end.** Math is near-saturating (probe 3→12→14; NLL floor
1.41 already ~reached) → predicted end **16–18/20** (clears its half). General is
below bar and NOT recovering: std stuck at 11 across a 2.2× token increase *and*
the 50:50→60:40 rebuild; **web-mode training loss went flat-to-rising (2.80→2.87)
and ticked *up* across the 50:50→60:40 switch** — more replay at 3e-4 did not help.
Predicted end **10–12/15 (sub-bar)**. Finishing ≈ 8 days for ~0 useful gain
(exactly §13's "math saturates early, forgetting doesn't").

**2. Weight-soup frontier (seed ⊕ intermix, both θ=1e6 → clean linear interp).**
Training-free; α=0 is seed, α=1 is raw intermix.

| α | MATH std/ext (comb/40) | GEN std/ext (comb/30) |
|---|---|---|
...
- **`0.35·seed + 0.65·intermix` clears the bar (MATH 15, GEN 13), robust on the
  fresh extension set (12/20 · 13/15), and beats the raw intermix on BOTH axes.**
- The raw checkpoint is **slightly over-trained** (α=0.70→0.75 give *more* math
  *and* general than α=1.0 — a WiSE-FT overshoot); pulling ~30% back toward the
  seed recovers the lost general for free. Math and general are **nearly linearly
  reconcilable in weight space**, and α≈0.65 is the sweet spot.

**3. Opportunity cost.** §15 already produced a working 4-quadrant reasoner on
off-the-shelf Qwen/Llama *without* intermix; the intermix+soup payoff is
specifically the **from-scratch-Argonne** ambition — which α≈0.65 now achieves
*at the base level*.

### Verdict & the plan (this is what is now implemented)
- **Worth the effort? YES.** Intermix broke the numeracy ceiling that beat every
  from-scratch approach in §1–§13 and — averaged with its seed — yields the first
  from-scratch Argonne base to clear the both-axes bar (15/13). It also replaced
  longmino (context extension) in the same phase.
- **Continue the 16B run? NO.** Math is banked/saturating; general won't reach the
  bar as-is. **Do instead:**
  1. **Stop / cancel the current intermix chain** (keep `363908` + seed).
  2. **Build the α≈0.65 soup** as the base (free, clears the bar today; a fine
     α∈{0.6,0.65,0.7} + extension recheck can lock it in).
  3. **Run the downstream SFT→DPO→CoT→4-quadrant on the soup** — the real
     "qualified" gold standard, *never tested on an Argonne base* (§15's link is
     borrowed from Qwen/Llama). This ~1-day 1×H100 run is the single decisive
     experiment.
  4. *Only if* downstream general falls short: a short recipe-changed slice
     (LR 3e-4→1e-4, GENERAL_RATIO=2.0) or finer α — but the probe already clears,
     so try downstream first.
- **Caveat:** the bar is a coarse base *probe* (easy math, keyword grading, ±1
  wobble; α=0.65 general is exactly at 13, though ext 13/15 corroborates).
  Clearing it is necessary, not sufficient — the 4-quadrant run is the honest judge.

### Implemented — the soup pipeline is now wired into the launchers
`weekend.sh` and `night.sh` gained a `MODE` switch defaulting to **`soup`** (the
old intermix run is preserved under `MODE=intermix` for the step-4 fallback):
- **`MODE=soup`** submits `reasoning/sft_soup.sh`, which **builds the soup base
  once** then **auto-chains** `sft_soup → dpo_soup → cot_soup → eval_soup`. Each
  stage is a 1×H100 continuous-run job that self-resubmits and chains the next
  (the same pattern as the §11 `_finemath` launchers), so the launchers just kick
  it off once — no weekend-style slice chain.
- **θ=1e6 throughout.** The soup and both its checkpoints are the argonne-3.0-base
  regime, so `cot_soup.sh` sets `ROPE_THETA=1000000.0` (NOT the FineMath 1e4 — the
  §11 corruption trap, reversed). SFT/DPO inherit θ from the base config.
- **New output dirs** (`sft_soup`, `dpo_soup`, `think_soup`) so nothing from
  §11/§12/§15 is clobbered. Eval logs → `report/soup_{math_nt,math_th,gen_nt,gen_th}.log`.
- **Operational gotcha:** the running INTERMIX chain does **not** stop on its own —
  each `midtraining.sh` slice resubmits the next *directly* (editing weekend.sh
  does not redirect it). `scancel` it to free the H200s (its math is already banked
  into the soup). The soup build reads the pinned `363908.pt`, which is retained.

**New files (this experiment).**
| File | What |
|---|---|
| `reasoning/build_soup_base.py` | Memory-frugal `(1-α)·seed + α·intermix` average → standalone HF base dir (θ=1e6, ctx 13568, trimmed embeddings). Idempotent; env: `ALPHA`, `SEED_CKPT`, `INTERMIX_CKPT`, `OUT`, `ROPE_THETA`, `TOKENIZER_SRC`. |
| `reasoning/sft_soup.sh` | Builds the soup base (once), runs general SFT (UltraChat) on it, chains → `dpo_soup.sh`. Entry point for `MODE=soup`. |
| `reasoning/dpo_soup.sh` | DPO (KatoHF chatbot_arena, `chat_refine_strict`) on the soup SFT model, chains → `cot_soup.sh`. |
| `reasoning/cot_soup.sh` | CoT-SFT on `cot_sft_mix_v3`, **θ=1e6**, chains → `eval_soup.sh`. |
| `reasoning/eval_soup.sh` | 4-quadrant eval of `think_soup` vs `think_mix3/mix2/star2`. The key A/B is `think_soup` vs `think_mix3` (same v3 CoT, OLD innumerate base) — isolating the both-axes soup base. |

**Reproduce the soup / eval.**
```bash
# 1. training-free soup base (α=0.65 is the default; θ=1e6 baked into config):
ALPHA=0.65 python reasoning/build_soup_base.py       # -> models/soup_seed_intermix_a065
# 2. probe the SOUP DIR on both axes (the go/no-go was: MATH ≥14 AND GEN ≥13):
EXTRA_CKPT=/project/rcc/youzhi/models/soup_seed_intermix_a065 EXTRA_THETA=1000000 \
  EXTRA_LABEL="soup a=0.65" python reasoning/base_probe_general.py
#    (a fuller extension-set eval: point eval_intermix_base.py's INTERMIX_CKPT at the soup dir.)
# 3. run the whole downstream pipeline on the soup (steps 1+2 are optional — sft_soup.sh
#    builds the soup itself; this is also exactly what `MODE=soup bash weekend.sh` fires):
sbatch reasoning/sft_soup.sh
```
...
§17 built the α=0.65 weight-soup base (probe **15/13**, first from-scratch Argonne base to clear the both-axes bar) and wired `weekend.sh MODE=soup` to run the full Argonne reasoning recipe on it. This section is **what actually happened when we ran it** — the *real* "qualified base" test that no Argonne base had ever taken. Bottom line up front:

> **The soup base broke the NUMERACY ceiling for good — `think_soup` is the first
> from-scratch Argonne model to score 10/10 in BOTH math modes and solve all four
> §10 residuals. But its GENERAL chat regressed under CoT-SFT (loops + lost facts),
> and rebalancing the CoT data (mix v4) did NOT fix it — a lateral trade. The
> from-scratch soup line plateaus at ~29/40 (strong math, weak general). A clean
> four-quadrant pass still belongs to §15's real off-the-shelf bases (~36/40).**

### 18a. What ran (zero-crash, end-to-end)
`MODE=soup bash weekend.sh` →
`build_soup_base.py` (0.35·seed329148 + 0.65·intermix363908, θ=1e6, trimmed embeds) →
`sft_soup.sh` (UltraChat, 1×H200) → `dpo_soup.sh` (KatoHF chatbot_arena, **204 pairs — no §12 zero-pairs crash**) → `cot_soup.sh` (**`cot_sft_mix_v3`, θ=1e6**, 3×H200 DDP) → `eval_soup.sh` (4-quadrant). The whole chain ran with no pipeline failures; θ=1e6 held through every stage (verified in the CoT log).
...
- **The math ceiling is gone.** `think_soup` is the ONLY Argonne model that solves math-no-think (every other degenerates — mix3 literally boxes an operand), and the only one at 10/10 *both* modes. It nails all four §10 residuals cleanly (`2x=17−5=12,x=6`; `n(n+1)/2=55`; `2·(8+3)=22`; `12=2²·3→(2+1)(1+1)=6`). Highest total of any model. Six months of STaR/GRPO/data-calibration never did this — the soup base did it immediately.
- **But general regressed:** GEN no-think 5/10, GEN+CoT 4/10 — worse than the old-base mix2/mix3 (~8). The failures are **non-terminating loops** on general prompts (grammar → "conjunction"×11; oxygen trace never closes) plus **lost facts** (Red Planet→Earth, primary colors→green). Classic §6 zero-sum diet — but notably *worse on the soup base than the SAME v3 data was on the old base* (mix3 kept 8/10 general).

### 18c. The diagnostic (the key methodological result)
Before rebuilding data, we localized the loss (throughline #8: eval and decide). We evaluated **`dpo_soup` — the model *before* CoT-SFT** — on general:

> **`dpo_soup` was general-HEALTHY: ~7–8/10, concise, NO loops.** It answered the
> grammar-correction *correctly* ("She doesn't like apples"), gave concise
> oxygen/refrigerator/photosynthesis answers, no degeneration.

So **general was fine coming out of SFT+DPO; the CoT stage broke it.** This is decisive: **the base is NOT the bottleneck — the CoT fine-tune is.** (Two errors — the taller/shorter logic puzzle, primary colors — are wrong in `dpo_soup` too, so *those* are genuine base-capability gaps that no CoT data fixes.) Consequence: **more midtraining / a better soup would not help** — it doesn't touch the CoT stage. The lever is the CoT step itself.

### 18d. mix v4 — rebalance the CoT data (DID NOT fix general; lateral trade)
`cot_sft_mix_v3` is 70% long-`<think>` rows, only 30% direct/no-think — on the (more plastic) soup base that long-trace dominance over-generalized into "always reason at length" → looping on general. Fix attempt (`build_mix_v4.py`): keep ALL of v3 (protect the math tiers) and **upsample the concise `direct_tulu` no-think tier 3× → 56% direct / 44% think**. Re-ran only the CoT stage from `dpo_soup` → `think_soup_v4`.

| quadrant | think_soup (v3) | **think_soup_v4** (v4) | mix2 (old base) |
|---|---|---|---|
...
- **Same 29/40 — a lateral trade** (+1 general/mode, −1 math/mode). mix v4 *fixed* Red Planet (→Mars) and closes more traces, **but the grammar-fix still loops in both modes** (56% concise data reduced but didn't eliminate it), and the base-level errors persist.
- **CONFOUND:** `think_soup_v4` changed *two* things vs `think_soup` — the data (v3→v4) **and** the batch/LR (eff-12/1e-5 → eff-30/1.6e-5, forced by the HBM-fill request below). So the −1 math may be the larger batch, not the data. Not a clean ablation.
- **Verdict: rebalancing CoT data does not rescue the soup base's general regression.** The from-scratch soup line plateaus strong-math / weak-general regardless of v3-vs-v4.

### 18e. HBM-fill saga (operational lesson: the loss-logits are the CoT memory ceiling)
Asked to run CoT at ≥95% HBM (batch 4 on short mix-v4 rows only used ~63%), we bumped the per-GPU micro-batch and hit a hard wall:
- **batch 18 → OOM, batch 16 → OOM, batch 12 → OOM** — all in the *backward* pass, not startup. The killer is the **fp32 loss logits `(batch × seq × vocab=151,669)`** that HF-Trainer materializes for cross-entropy (the §15 "logits are the ceiling" lesson): at seq≈4k each batch-row costs ~4 GiB and the backward briefly needs ~30+ GiB for that one tensor. Startup HBM (~92%) is *not* the constraint; the backward is.
- **batch 10 = 98% HBM, no OOM** across the full ~9.8 h / 6k-step epoch. That's the ceiling for this stage.
- **Lessons for the next agent:** (1) on CoT-SFT with this huge vocab + long ctx, the *backward-pass loss-logits* set the batch ceiling — profile the backward, not startup. (2) grad-checkpointing is ON (hard-coded in `cot-sft.py`) and CANNOT be disabled here (activations for a 4k batch would OOM). (3) grad-accum is already 1, so filling HBM forces a bigger micro-batch → a **bigger effective batch** (quality tension: eff-30 here vs the proven eff-12). Fill HBM only if you accept that trade; otherwise batch 4 (eff-12, proven recipe) is the quality-first choice at ~63% HBM.

### 18f. Decoding hypothesis — REFUTED (2026-07-05, `report/soup_dec_*.log`)
The biggest general losses — the grammar loop and the unclosed with-CoT traces — *looked* like an **inference/decoding** artifact, so we added `--repetition-penalty` / `--no-repeat-ngram` to `eval_numeracy.py` (defaults off; penalize GENERATED tokens only, never the prompt — §5) and re-ran all 4 quadrants for both soup models at **rep-penalty 1.3 + no-repeat-3**. Result:
...
- **Net LOSS. The anti-repeat penalty CORRUPTS math** — arithmetic legitimately repeats digits/operators, so the penalty blocks re-emitting them: `half of 80` decoded as "80/2" → "**8**/2" (the "0" got penalized) → boxed **4**; `15% of 80` → "15/100 × **8**" → **9**. Math collapses 10→4. It also garbles natural sentences (grammar-fix → "doesn't liked").
- **It only helps in ONE cell** (`think_soup` general-no-think 5→7, loops suppressed) and does **not** recover general-with-CoT (stays 3) — the with-CoT and content failures (Red Planet→Earth, Bob→Tom, colors→green) are **real, not loops**.
- **Verdict: there is NO single decoding config that wins.** rep-penalty is a targeted anti-loop tool that is *ruinous* for a model whose only strength is arithmetic. **The best decoding for the soup line is plain GREEDY** (protects the 10/10 math); the general regression is baked into the CoT'd weights + a genuine base-knowledge gap, NOT a decoder artifact. The loops were partly real (suppressible) but the cure costs more than the disease. `think_soup` @ greedy = **29/40 (strong-math specialist)** is the real number.

### 18g. Verdict & what to do next
1. **Do NOT resume the intermix midtraining.** It's saturated (§16/§17: math maxed ~2B tokens, general won't move at LR 3e-4) AND the §18c diagnostic proved the *base isn't the bottleneck* — the CoT stage is. Midtraining spends compute on the wrong stage.
2. **Check the decoding re-eval (§18f) first** — cheapest possible test of whether we already have a good model.
3. **The durable path to a *clean* from-scratch reasoner is base QUALITY (throughline #1), not more argonne3.0 work.** §15 proved this exact recipe yields ~36/40 on a Qwen/Llama-grade base; the soup was a clever *repair* but the underlying 2.88B base still isn't that grade. This is what the **argonne3.5 recipe search** (higher LR + cooldown + fp8 + better tokens) targets — a better-pretrained base makes the reasoning recipe just work.
4. *If* one insists on improving the soup reasoner: the un-confounded test is re-running v4 CoT at the **proven eff-12 / LR-1e-5** (accepting ~63% HBM) to isolate the data effect — but expect at best a small general gain, since the loop pathology and base-level errors persist.

### New files / changes this session
| File | What |
|---|---|
| `reasoning/build_soup_base.py` | Training-free `(1-α)·seed + α·intermix` weight-soup → HF base dir (θ=1e6, ctx 13568, trimmed embeds, idempotent). |
| `reasoning/{sft,dpo,cot,eval}_soup.sh` | Soup downstream chain (copies of the §11 `_finemath` launchers; **`cot_soup.sh` θ=1e6**, defensive `unset` for the §12 leak; dirs `sft_soup`/`dpo_soup`/`think_soup`; logs `report/soup_*.log`). |
| `reasoning/build_mix_v4.py` | mix v4 = v3 + 3× upsampled `direct_tulu` (56% concise no-think) → `cot_sft_mix_v4`. |
| `reasoning/{cot,eval}_soup_v4.sh` | v4 CoT (batch 10 @98% HBM, LR 1.6e-5) + eval → `think_soup_v4`, `report/soup_v4_*.log`. |
| `reasoning/eval_numeracy.py` (edited) | Added `--repetition-penalty` / `--no-repeat-ngram` to the decode loop (backward-compatible; generated-tokens-only). |
| `weekend.sh` / `night.sh` (edited, §17) | `MODE` switch: default `soup` runs the pipeline above; `MODE=intermix` keeps the old midtraining chain. |

Models retained (as of §19): `models/soup_seed_intermix_a065` (base), `models/instruct/{dpo_soup,think_soup}` (needed to rebuild the §19 blend), and **`models/instruct/soup_blend_a085`** (the FINAL reasoning model). `sft_soup` (consumed by DPO) and `think_soup_v4` (superseded by the blend) were deleted.

---

## 19. Training-FREE general recovery — weight-soup the pre-CoT and post-CoT checkpoints (2026-07-05)

§18c localized the damage precisely: **`think_soup = dpo_soup + a CoT weight-delta` in ONE optimization basin** (`cot_soup.sh` trains `think_soup` *from* `dpo_soup`, same arch, same θ=1e6), the pre-CoT `dpo_soup` was **general-healthy (~7–8/10)**, and the CoT stage is what overwrote general. §18f then refuted the decoding fix. That leaves one cheap, un-tried lever that attacks the *weights* instead of the data or the decoder:

> **Linearly interpolate the two same-lineage checkpoints:
> `blend_α = (1−α)·dpo_soup + α·think_soup = dpo_soup + α·(CoT-delta)`.**
> High α keeps `think_soup`'s 10/10 math; low α restores `dpo_soup`'s general. This
> is **training-FREE** (minutes of tensor-averaging on CPU, no GPU steps) — the same
> model-soup trick that built the soup base in §17, now applied one stage later to
> undo the CoT stage's general regression *by fractionally un-applying it*.

- **`reasoning/build_ckpt_soup.py`** — memory-frugal (safetensors `safe_open`, one key at a time, fp32 blend) averager of two HF dirs; copies config + tokenizer (the chat template that carries think-mode) from the THINK dir; idempotent.
- **`reasoning/soup_recover.sh`** — builds `blend_a{050,070,085}` then runs the SAME 4-quadrant probe (**greedy no-think / sampled think, NO rep-penalty** — §18f) over the three blends + `think_soup` (α=1 anchor). 1×H100.
- **Hypothesis / go bar:** an α exists where general-no-think climbs back toward `dpo_soup`'s ~7–8 while math stays ≥8/10 both modes → a genuine all-round from-scratch reasoner (>29/40, *balanced*).

### 19a. RESULT — it WORKS. `blend_a085` = 33/40, the best from-scratch Argonne reasoner (2026-07-05, `report/recover_*.log`)
...
<sub>*think_soup re-graded 31 here vs §18b's 29 — GEN+CoT is *sampled*, so ±1–2 run-to-run; the deterministic quadrants (math_nt, gen_nt) are stable.*</sub>

- **`soup_blend_a085` (α=0.85) is the FINAL reasoning model.** It keeps `think_soup`'s **perfect 10/10 math in BOTH modes** *and* recovers general no-think **5→7** — the **grammar loop is gone** ("She doesn't like apples", clean) and the **lost Mars fact is restored** — for **+2 net over the 29–31 plateau, now BALANCED**. Training-FREE (CPU tensor-average, ~2 min/blend). First from-scratch Argonne model that is simultaneously a perfect arithmetic reasoner and a loop-free generalist.
- **The α curve tells a clean mechanistic story.** *Lower* α suppresses more no-think loops (a050 gen-nt 8, zero loops) BUT **breaks CoT trace-closure** — a050/a070 gen-with-CoT collapse to **1/10** because the `<think>…</think>` *formatting itself lives in the CoT weight-delta*; dilute it too much and traces never close (`closed=False`, right answer trapped inside). a050 also loses math-with-CoT (5/10, unclosed). So there's a floor on how much you can dilute. **α=0.85 is the knee:** just enough `dpo_soup` to fix the general loops + facts, still enough CoT-delta to keep 10/10 math *and* close traces (gen+CoT 6, = anchor).
- **Residual misses are genuine base-capability gaps, NOT fixable by any soup/decoding/data lever:** primary-colors→"green" (should be yellow) and the taller/shorter transitivity puzzle are **wrong in `dpo_soup` itself** (§18c). These are the 2.88B base's real ceiling — exactly what the **argonne3.5 base-quality search** exists to raise.

### 19b. FINAL VERDICT — the reasoning-model line is DONE
1. **Ship `models/instruct/soup_blend_a085`** (greedy for math/no-think; sampling fine for think-mode). It is the best from-scratch Argonne reasoner: **10/10 math (both modes) + recovered general, 33/40.**
2. **The full recipe that produced it** (reproducible, all training-free after DPO): seed⊕intermix soup base (§17) → SFT → DPO (`dpo_soup`) → CoT-SFT mix-v3 (`think_soup`) → **weight-soup `0.15·dpo_soup + 0.85·think_soup`** (§19). The last step is the novel bit: it *surgically un-does* the CoT stage's general regression while keeping its math, in one basin, for free.
3. **Do NOT** resume intermix midtraining (§18g), chase decoding (§18f), or re-balance CoT data (§18d) — all three were tried and are inferior to the weight-soup recovery.
4. **The only remaining lever for a *cleaner* reasoner is base QUALITY** — the residual general misses are base gaps (throughline #1). That is the argonne3.5 recipe work, not more argonne3.0.
5. *Optional marginal refinement (not pursued — diminishing returns):* the peak may sit at α∈(0.80, 0.90); a one-job sweep could squeeze +0–1. Not worth it against 33/40 with perfect math.

Retained: `models/instruct/soup_blend_a085` (**final model**), `dpo_soup` + `think_soup` (needed to rebuild the blend). Deleted the losing blends (`a050`, `a070`), `sft_soup` (consumed by DPO), and `think_soup_v4` (superseded lateral trade) to reclaim space.
