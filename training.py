...
# Intel XPU optimizations for Intel Data Center GPUs
...
def train_model_parallel(data_files, use_streaming=False, use_compile=False):  # Set default to False
    """
    ...
    Args:
        ...
        use_compile: Whether to use torch.compile() for model optimization - DISABLED FOR INTEL XPU
    """
    ...
    while True:
        ...
        try:
            ...
            print(f"Available Intel GPUs: {num_gpus}")
            ...
            # Apply torch.compile() for speed optimization - DISABLED FOR INTEL XPU
            print("torch.compile() is disabled for Intel XPU compatibility")
            
            optimizer = torch.optim.AdamW(model.parameters(), lr=3e-5, weight_decay=0.1)
            # Properly optimize both model and optimizer for Intel XPUs
            model, optimizer = ipex.optimize(model, optimizer=optimizer)
            
            scaler = torch.amp.GradScaler()  # Standard GradScaler
            global_step = 0
                    ...
                    while step_in_epoch < steps_per_epoch:
                        try:
                            ...
                            if len(token_batch) == batch_size:
                                ...
                                optimizer.zero_grad()
                                # Use recommended autocast syntax
            ...
            if "out of memory" in str(e).lower():
                ...
                print(f"XPU Out of Memory! Reducing batch size from {batch_size} to {new_batch_size}")
...
def main():
    ...
    # Set use_compile=False to avoid Intel XPU compiler issues
    train_model_parallel(data_files=data_files, use_streaming=True, use_compile=False)
