...
class ArgonneConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # block_size counts sub-layers (attn + mlp); 4 means 2 transformer layers per block
...
def block_attn_res(
    ...
    Args:
        blocks: list of [B, T, D] tensors (completed block reps + embedding)
        partial_block: [B, T, D] intra-block partial sum, or None
        query: [D] learned pseudo-query vector
        norm: RMSNorm applied to keys
    Returns:
        h: [B, T, D] aggregated representation
    """
    ...
    # weights: [N, B, T], V: [N, B, T, D] -> h: [B, T, D]
...
class Block(nn.Module):
    ...
    def __init__(self, config: ArgonneConfig, layer_idx: int = 0) -> None:
        ...
        # Attention Residuals: per-layer pseudo-queries and key norms
        self.use_attn_res = getattr(config, 'use_attn_res', False)
        if self.use_attn_res:
            d = config.hidden_size
            # Pseudo-queries initialized to zero (critical for stability)
            self.attn_query = nn.Parameter(torch.zeros(d))
            self.mlp_query = nn.Parameter(torch.zeros(d))
            # RMSNorm on keys prevents large-magnitude blocks from dominating
            self.attn_key_norm = RMSNorm(d, eps=config.rms_norm_eps)
            self.mlp_key_norm = RMSNorm(d, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        """Standard residual forward (used when use_attn_res=False)."""
...
class ArgonneModel(PreTrainedModel):
    ...
    def __init__(self, config: ArgonneConfig) -> None:
        ...
        # Attention Residuals: final output aggregation
        self.use_attn_res = getattr(config, 'use_attn_res', False)
        if self.use_attn_res:
            d = config.hidden_size
            self.final_attn_query = nn.Parameter(torch.zeros(d))
            self.final_key_norm = RMSNorm(d, eps=config.rms_norm_eps)
            # Transformer layers per AttnRes block
            self.layers_per_block = config.attn_res_block_size // 2
    ...
    def _init_weights(self, module: nn.Module) -> None:
        ...
        # Note: nn.Parameter pseudo-queries are initialized to zero in Block.__init__
        # and are not touched here since they are not nn.Module subclasses.
    ...
    def _forward_standard(
        ...
        """Standard residual forward path."""
    ...
    def _forward_attn_res(
        ...
        """Attention Residuals forward path (Block AttnRes)."""
        # b0 = token embedding, always included as a source
        blocks_list = [hidden_states]
        partial_block = None

        for layer in self.blocks:
            idx = layer.layer_idx

            # --- Pre-attention: compute h via depth-wise attention ---
            h = block_attn_res(
                blocks_list, partial_block,
                layer.attn_query, layer.attn_key_norm,
            )

            # Block boundary: append completed block, reset partial
            if idx > 0 and idx % self.layers_per_block == 0:
                blocks_list = blocks_list + [partial_block]
                partial_block = None

            # Attention sub-layer
            attn_out = layer.attn(layer.input_norm(h), rotary, attention_mask)
            if partial_block is not None:
                partial_block = partial_block + attn_out
            else:
                partial_block = attn_out

            # --- Pre-MLP: compute h via depth-wise attention ---
            h = block_attn_res(
                blocks_list, partial_block,
                layer.mlp_query, layer.mlp_key_norm,
            )

            # MLP sub-layer
            mlp_out = layer.mlp(layer.post_norm(h))
            partial_block = partial_block + mlp_out

        # Final output: aggregate all blocks + remaining partial
        hidden_states = block_attn_res(
            blocks_list, partial_block,
            self.final_attn_query, self.final_key_norm,
        )
        return hidden_states
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.use_attn_res:
            hidden_states = self._forward_attn_res(hidden_states, rotary, attention_mask)
        else:
            hidden_states = self._forward_standard(hidden_states, rotary, attention_mask)
